...
export async function createShortcutStory(
  ...
  const SHORTCUT_TOKEN = core.getInput("clubhouse-token", { required: true });
  ...
  const clubhouseUserId = await getShortcutUserId(githubUsername, http);
  const clubhouseProject = await getShortcutProjectByName(PROJECT_NAME, http);
  if (!clubhouseProject) {
...
export async function getShortcutStoryIdFromPullRequest(
  ...
  const clubhouseURL = await getShortcutURLFromPullRequest(payload);
  if (!clubhouseURL) {
    return null;
  }

  const match = clubhouseURL.match(SHORTCUT_STORY_URL_REGEXP);
  if (match) {
    return match[1];
  }
  return null;
